...
async def create_job(job: dict, user=Depends(require_role("admin"))):
    return {"id": "new-id", **job}

@router.patch("/{job_id}", summary="Update job")
async def update_job(job_id: str, patch: dict, user=Depends(require_role("admin"))):
    return {"id": job_id, **patch}

@router.delete("/{job_id}", summary="Delete job")
async def delete_job(job_id: str, user=Depends(require_role("admin"))):
    return {"deleted": job_id}

@router.get("/{job_id}/candidates", summary="Matched candidates for a job")
async def matched_candidates(job_id: str, user=Depends(require_role("admin"))):
